<script lang="ts">
  ...
  $: {
    if (!stateMap || Object.keys(stateMap).length === 0) {
      stateMap = {
        ...
        'llm_processing': { active: '', processing: 'llm', completed: ['start', 'mic-on', 'vad', 'stt'] },
        'mcp_processing': { 
          active: '', 
          processing: 'mcp', 
          completed: ['start', 'mic-on', 'vad', 'stt', 'llm'] 
        },
        'tts_processing': { 
          active: '', 
          processing: 'tts', 
        ...
        'tts_processing_with_mcp': { 
          active: '', 
          processing: 'tts', 
          completed: ['start', 'mic-on', 'vad', 'stt', 'llm', 'mcp'] 
        },
        'responding': { 
          active: '', 
        ...
        'responding_with_mcp': { 
          active: '', 
          processing: 'audio-response', 
          completed: ['start', 'mic-on', 'vad', 'stt', 'llm', 'mcp', 'tts'] 
        }
      };
    }
  }
  ...
  $: {
    ...
    if (currentState && stateMap[currentState.toLowerCase()]) {
      ...
      // Debug flow state transitions (can be removed after testing)
      // console.log(`🔄 Flow State: ${currentState} -> Processing: ${processingNode}, Completed: [${completedNodes.join(', ')}]`);
