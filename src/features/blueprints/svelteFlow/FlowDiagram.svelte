    ...
    if (activeNode || processingNode || completedNodes.length > 0 || currentState !== 'idle' || isRunning) {
      nodes.update(currentNodes => {
        return currentNodes.map(node => {
          ...
          // Debug MCP node specifically (can be removed after testing)
          // if (node.id === 'mcp') {
          //   console.log(`🔍 MCP Node State - Active: ${isActive}, Processing: ${isProcessing}, Completed: ${isCompleted}, CompletedNodes: [${completedNodes.join(', ')}]`);
          // }
